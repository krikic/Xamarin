...
public class ParState extends GameState
{
    ...
    public ParState(ParState original)
    {
       ...
       // playerNames = setNames(original.playerNames);
        //board = setBoard(original.board);
       // dice = setDice(original.dice);
    ...
    public int[] setDice(int[] initDice){
    	int newDice[] = new int[2];
    	for(int i =0; i<initDice.length;++i){
    		newDice[i]=initDice[i];
    	}
    	return newDice;
    }
    ...
    public int[][] setBoard( int[][] initBoard){
    	int newBoard[][] = new int[4][77];
    	for(int i=0; i<initBoard.length; ++i){
    		for(int j= 0; j<initBoard[i].length;++i){
	...
	public String[] setNames(String[] initNames){
		for(int i = 0; i<4; ++i){
