...
{    
	...
	{    
		...
		/// <summary>    
		/// Encrypts given data using symmetric algorithm AES, uses HASH512 ready to be sent through Webservice    
		/// </summary>    
		/// <param name="data">Data to encrypt</param>    
		/// <param name="password">Password</param>    
		/// <param name="salt">Salt</param>    
		...
		public static string EncryptToSend(string data,byte[] salt)
		{
			
			var pass = "soutoFTW";

			byte[] encryptedpw = Crypto.EncryptAes(data,pass,salt);
			var hasher = WinRTCrypto.HashAlgorithmProvider.OpenAlgorithm(HashAlgorithm.Sha512);
			byte[] hash = hasher.HashData(encryptedpw);
